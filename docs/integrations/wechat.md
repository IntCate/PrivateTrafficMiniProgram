...
| 商户 API 证书（apiclient_cert / key） | 请求签名 |
...
- `POST /api/orders/{id}/refund`（api-design §9.7）先落 `orders` 表（置 status=refund + refund_* 字段）；
...
| `docs/sql/schema.sql` | member / member_session / orders 表 |
