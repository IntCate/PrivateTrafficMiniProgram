...
- 手机号、token、openid、session_key 一律脱敏（见 [logging.md](conventions/logging.md)）。

### 5.6 定时任务（APScheduler）

| 任务 | 周期 | 说明 |
| --- | --- | --- |
| 订单超时关闭 | 每 1 分钟 | `orders.status=pending` 且超 30 分钟未支付 → 关闭 + 回补 `lock_stock` |
...
| after_sale | 申请售后、售后单查询（P1） | api-design §12 预留 |
